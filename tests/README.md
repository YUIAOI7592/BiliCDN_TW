...
目前 `npm test` 功能套件 354 項、無 skip；依使用者要求，v1.8.3～v1.9.3 不執行獨立 CS 子集或 Code Security。v1.9.3 新增 restriction freshness gate、跨分頁到期同步、零尺寸影片 resolver、SPA resolver reset 與移除無效 BroadcastChannel 的固定重現；v1.9.2 的長暫停核心恢復、v1.9.1 的起播保護及 v1.9.0 的播放器 manifest／MPD 重建案例仍保留。歷史安全與舊版重現檔仍保留，但不以其結果替目前版本背書。
