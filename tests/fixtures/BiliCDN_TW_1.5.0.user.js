...
// 動態調整請使用 Tampermonkey 的「切換 catalog 自動選路節點」選單。
//
...
// Tampermonkey 選單的「顯示 Worker 使用量」中 mediaSeen > 0 才代表 Worker 真的抓過影片分段。
...
// 但至少讓 Tampermonkey 的「顯示診斷」能一眼看出「是選擇器找不到面板，還是真的沒問題」。
...
const noteSegmentBytes = (cdn, xhr, startedAt, url, alreadyReportedBytes, runtimeToken) => {
    ...
    try {
        ...
        if (!bytes) return
        noteObservedVideoRepresentation((xhr.responseURL && isMediaSegmentUrl(xhr.responseURL)) ? xhr.responseURL : url)
...
const rebuildRepresentationRegistry = () => {
    ...
    streamProfile.reps.forEach(rep => {
        ...
        const safeRep = {
            height: Math.max(0, Math.trunc(+rep.height || 0)),
        ...
        ;(Array.isArray(rep.urls) ? rep.urls : []).forEach(url => {
            ...
            if (!key) return
            if (!representationRegistry.has(key) && representationRegistry.size >= REPRESENTATION_REGISTRY_MAX) {
                representationRegistry.delete(representationRegistry.keys().next().value)
            }
            representationRegistry.set(key, safeRep)
        })
    })
}
const noteObservedVideoRepresentation = (url) => {
    const key = representationIdentity(url)
    const rep = key && representationRegistry.get(key)
    if (!rep) return false
...
const streamUrlIdentity = (urlStr) => {
    try { return new URL(urlStr).href } catch { return String(urlStr || '') }
}
...
const sanitizePlayInfoUrls = (root) => {
    ...
    const rewrite = (value) => {
        ...
        if (preservedOriginalStreamUrls.has(value)) return value
...
const getCodecCapabilityState = (kind, height) => {
    ...
    const heightKey = height >= 2160 ? 2160 : 1080
    const cap = codecCapability.get(kind + '-' + heightKey)
    if (!cap || typeof cap !== 'object') return 'unknown'
    if (cap.supported === false || cap.smooth === false || cap.powerEfficient === false) return 'bad'
    if (cap.supported === true && cap.smooth === true && cap.powerEfficient === true) return 'good'
    return 'unknown'
}
...
// decodingInfo() 是非同步的，但 playurl 到達時要同步排序；document-start 先預熱常見組合，
// 尚未有答案時以 unknown 處理，representation 明確可播放就尊重使用者 codec 偏好。
...
const normalizeDashCodecPreference = (dash) => {
    ...
    const codecRank = (item) => {
        ...
        const capability = getCodecCapabilityState(kind, heightKey)
    ...
    groups.forEach(group => {
        ...
        if (decisions.length < 8 && selected) {
            ...
            decisions.push({
                ...
                selected: kind,
                capability: getCodecCapabilityState(kind, height),
            })
        }
    })
...
const interceptNetResponse = (function (theWindow) {
        ...
        send(...args) {
            ...
            if (this._originCdn) {
                ...
                this.addEventListener('abort', (e) => {
                    if (e && e.isTrusted === true && self._biliRequestSeq === requestSeq) aborted = true
                }, { once: true })
                ...
                this.addEventListener('progress', (e) => {
                    ...
                    if (delta > 0) {
                        ...
                        noteObservedVideoRepresentation((self.responseURL && isMediaSegmentUrl(self.responseURL)) ? self.responseURL : self._interceptUrl)
                ...
                this.addEventListener('readystatechange', function (e) {
                    ...
                    if (HARD_FAIL_STATUSES.has(self.status)) {
                        ...
                        noteSegmentBytes(cdn, self, durationBase, self._interceptUrl, lastProgressLoaded, requestRuntimeToken)
                    }
                })
            }
    ...
    const wrapMeasuredFetchResponse = (res, cdn, effectiveUrl, fetchStartedAt, failureContext, requestRuntimeToken) => {
        if (!res.body || typeof res.body.getReader !== 'function' || typeof ReadableStream === 'undefined') {
            if (isRuntimeGenerationActive(requestRuntimeToken)) {
                noteObservedVideoRepresentation(res.url || effectiveUrl)
    ...
    theWindow.fetch = (input, init) => {
        ...
        if (isMediaSegmentUrl(urlStr)) {
            ...
            return OriginalFetch(fetchInput, init).then(res => {
                ...
                if (res.ok) return wrapMeasuredFetchResponse(res, targetCdn, effectiveUrl, fetchStartedAt, failureContext, requestRuntimeToken)
        ...
        return OriginalFetch(input, init).then(response =>
            response.text().then(text => {
                let out = text
                try {
                    const transformed = handleInterceptedResponse(text, urlStr)
                ...
                try {
                    return cloneResponseWithBody(response, nullBody ? null : out, false)
                } catch (e) {
...
const handleVerifiedSegmentFailure = ({
    ...
    if (lastSampleSegmentUrl && (currentStreamBitsPerSec / 1e6 >= 12 || playbackRateState.effectiveRate >= 1.75)) {
        runThroughputBakeoff(lastSampleSegmentUrl, false, trustedBakeoffRequest('verified-failure')).catch(() => {})
    }
    return true
}
...
const biliCdnIsUnstable = (host) =>
    !!host && (/\\.mcdn\\.bilivideo\\.(cn|com)$/i.test(host)
        ...
        || (/^cn-[a-z]{2,8}-/i.test(host) && host.endsWith('.bilivideo.com')));
...
const biliCdnIsMedia = (url) => {
    ...
    try {
        const u = new URL(url);
        const host = u.hostname;
        if (!(host.endsWith('.bilivideo.com') || host.endsWith('.bilivideo.cn') || biliCdnIsUnstable(host))) return false;
        return u.pathname.endsWith('.m4s') || u.pathname.endsWith('.flv') || u.pathname.includes('/upgcxcode/');
    } catch { return false; }
};
const biliCdnNeedsRedirect = (host) =>
    !!host && (biliCdnIsUnstable(host) || BILICDN_EXCLUDED.indexOf(host) !== -1
...
const biliCdnRewrite = (url) => {
    try {
        ...
        if (!biliCdnNeedsRedirect(parsed.hostname) || parsed.hostname === BILICDN_TARGET_HOST) return absolute;
...
const probeCdnThroughput = (cdn, sampleUrl, probeBytes, externalSignal) => new Promise((resolve) => {
    ...
    const to   = scheduleRuntimeTimeout(() => { try { ctrl.abort() } catch {} }, THRPT_PROBE_TIMEOUT)
    const t0   = performance.now()
    let ttfb   = 0
    let bytes  = 0
    let settled = false
    ...
    interceptNetResponse.rawFetch(target, {
        ...
        const pump = () => reader.read().then(({ done: rdone, value }) => {
            if (rdone || bytes >= wantBytes) {
                ...
                if (bytes >= THRPT_PROBE_MIN_BYTES && !(externalSignal && externalSignal.aborted) && !disabled) {
                    recordCdnThroughput(cdn, bytes, dl, playbackRateState.effectiveRate)
...
const runThroughputBakeoff = async (sampleUrl, skipIfFast = true, trustedRequest = null) => {
    ...
    if (trustedReason) {
        ...
        if (now - last < TRUSTED_BAKEOFF_MIN_GAP[trustedReason]) return
...
const Watchdog = (() => {
    ...
    const onEntry = (entry) => {
        ...
        try {
            ...
            noteObservedVideoRepresentation(entry.name)
    ...
    const switchCdn = (reason) => {
        ...
        if (currentStreamBitsPerSec / 1e6 >= 12 && lastSampleSegmentUrl) {
            runThroughputBakeoff(lastSampleSegmentUrl, false, trustedBakeoffRequest('watchdog')).catch(() => {})
        }
    ...
    const tick = () => {
        const v = getVideo()
        if (!v) { resetPlaybackRateState(); return }
    ...
    return {
        ...
        stats() {
            ...
            const videoTimeSec = v ? +(v.currentTime || 0).toFixed(2) : 0
            const bufferedEndSec = +bufferedEnd(v).toFixed(2)
...
const TrustedMenuUI = (() => {
    ...
    const ensure = () => {
        ...
        try {
            ...
            dom.addEvent(shadow, 'keydown', event => {
                if (!event || !event.isTrusted || event.key !== 'Escape' || !activeModal) return
                closeDialog()
            })
    ...
    const closeDialog = (restoreFocus = true) => {
        ...
        if (restoreFocus && previousFocus) {
            try { dom.focus(previousFocus) } catch {}
        }
        previousFocus = null
    }
    ...
    const beginDialog = ({ title, paragraphs = [], requiresCapability = false }) => {
        ...
        const focusToRestore = activeModal && previousFocus ? previousFocus : document.activeElement
        closeDialog(false)
        ...
        previousFocus = focusToRestore || null
    ...
    return Object.freeze({ toast, openConfirm, openChoice, openText, closeDialog, invalidate })
})()
...
const copyDiagReport = async () => {
    ...
    try {
        ...
        try {
            ...
            console.log(text)
            console.log('[BiliCDN] 自動複製失敗，已開啟手動複製視窗：', clipboardError)
...
const BiliCDNControls = {
    diag() {
        ...
        console.log('候選順序（初始 probe）:', activeCdnList.map(c => c.split('.')[0]))
        console.log('真正可選節點:', getHealthyCdnList().map(c => c.split('.')[0]))
        console.log('Catalog 自動選路狀態:', Object.fromEntries(TRUSTED_CDN_CATALOG.map(host => [host, isCatalogAutoEnabled(host)])))
        ...
        console.log('播放倍速:', { ...playbackRateState })
        console.log('串流估計（觀察到的 representation，不代表已確認解碼器）:', { ...streamEstimate })
        console.log('Codec 能力:', {
            preference: resolvedVideoCodecPreference,
            selectedMeaning: '排序首位（不是已確認解碼 codec）',
            ...
            recent: lastCodecDecision.groups,
        })
        ...
        console.log('WebRTC 阻擋:', BlockWebRTC && !disabled)
    ...
    async bakeoff() {
        ...
        await runThroughputBakeoff(lastSampleSegmentUrl, false, trustedBakeoffRequest('menu'))
        ...
        return controlResult(true, samples.length ? 'completed' : 'no-candidates',
            samples.length ? ('測速完成；目前最佳：' + getCdnShortName()) : '本輪沒有可量測候選', {
    ...
    verbose(on) {
        if (typeof on !== 'boolean') {
            console.log('[BiliCDN] Verbose =', Config.verbose,
                '\n請由 Tampermonkey「開啟／關閉 verbose」選單切換')
        ...
        return controlResult(true, on ? 'enabled' : 'disabled', 'Verbose 已' + (on ? '開啟' : '關閉'), {
            enabled: on,
        })
    },
    reset() {
    ...
    revive(host) {
        if (!host) {
            console.log('請使用 Tampermonkey「救回單一 dead catalog 節點」選單')
    ...
    exclude(kw) {
        if (!kw || typeof kw !== 'string') {
            console.log('請使用 Tampermonkey「切換 catalog 自動選路節點」選單')
...
const refreshPublicDiagnosticSnapshot = () => {
    try { publicDiagnosticSnapshot = buildPublicDiagnosticSnapshot() } catch {}
...
const showBlockedStateResetDialog = () => TrustedMenuUI.openConfirm({
    title: '重置黑名單與 dead 節點？',
    paragraphs: ['只清除 blacklist 與持久 dead host；健康樣本、固定 CDN、catalog override 與 HTTPDNS 學習不變。'],
    confirmLabel: '確認清除',
    danger: true,
    onConfirm: () => {
        clearBlacklist()
        ...
        syncWorkerCdnTarget()
        refreshPublicDiagnosticSnapshot()
        TrustedMenuUI.toast('黑名單與 dead 節點已清除', 'success')
    },
})
...
const showFixedCdnDialog = () => {
    const choices = [
        { label: '自動選路', detail: resolvedCdn ? '目前未使用' : '目前使用中' },
        ...TRUSTED_CDN_CATALOG.map(host => ({
            label: host,
            detail: [
                host === resolvedCdn ? '目前固定' : '',
                knownDeadHosts.has(host) ? 'dead' : '',
                isPresumedDnsFailHost(host) ? 'presumed；固定實測是唯一翻案方式' : '',
            ].filter(Boolean).join('；') || '可信 catalog 節點',
        })),
    ]
    const selected = [resolvedCdn ? Math.max(1, TRUSTED_CDN_CATALOG.indexOf(resolvedCdn) + 1) : 0]
    return TrustedMenuUI.openChoice({
        title: '固定 CDN／恢復自動',
        paragraphs: ['選擇後會重新載入頁面。固定 CDN 優先於 catalog override。'],
        choices,
        selected,
        confirmLabel: '套用並重新載入',
        onConfirm: picked => {
            const index = picked[0]
            if (!Number.isInteger(index) || index < 0 || index > TRUSTED_CDN_CATALOG.length) {
                TrustedMenuUI.toast('請選擇一個有效項目', 'error')
                return
            }
            const host = index === 0 ? null : TRUSTED_CDN_CATALOG[index - 1]
            const result = BiliCDNControls.setCdn(host)
            refreshPublicDiagnosticSnapshot()
            TrustedMenuUI.toast(result.message, result.ok ? 'success' : 'error')
...
const showCatalogDialog = () => {
    const selected = TRUSTED_CDN_CATALOG.map((host, index) => isCatalogAutoEnabled(host) ? index : -1).filter(index => index >= 0)
    const choices = TRUSTED_CDN_CATALOG.map(host => {
        const headerDefault = !matchesHeaderExclude(host)
        const flags = [
            '檔頭預設=' + (headerDefault ? '啟用' : '停用'),
            Object.prototype.hasOwnProperty.call(catalogOverrides, host) ? 'override' : '',
            knownDeadHosts.has(host) ? 'dead' : '',
            isPresumedDnsFailHost(host) ? 'presumed；自動選路會略過，請先固定實測' : '',
            host === resolvedCdn ? '目前固定' : '',
        ].filter(Boolean)
        return { label: host, detail: flags.join('；') }
    })
    return TrustedMenuUI.openChoice({
        title: 'Catalog 自動選路節點',
        paragraphs: [resolvedCdn
            ? '目前使用固定 CDN；此處變更會在恢復自動選路後生效。'
            : '至少必須保留一個非 presumed 候選。dead／presumed 狀態不會因勾選而自動解除。'],
        choices,
        multiple: true,
        selected,
        confirmLabel: '套用',
        secondaryLabel: '恢復檔頭預設',
        onSecondary: () => {
            const result = resetCatalogOverrides()
            TrustedMenuUI.toast(result.message, 'success')
        },
        onConfirm: picked => {
            const result = applyCatalogSelection(picked)
            TrustedMenuUI.toast(result.message, result.ok ? 'success' : 'error')
        },
    })
}
...
if (typeof GM_registerMenuCommand === 'function') {
    GM_registerMenuCommand('🔄 重置所有學習狀態', showResetLearningDialog)
    GM_registerMenuCommand('📊 顯示診斷資訊', showDiagnosticDialog)
    GM_registerMenuCommand('📋 複製診斷報告', () => BiliCDNControls.report())
    GM_registerMenuCommand('🏁 立即測速選節點', () => showAsyncControlResult('開始測速選節點…', () => BiliCDNControls.bakeoff()))
    GM_registerMenuCommand('📡 手動延遲探測', () => showAsyncControlResult('開始延遲探測…', () => BiliCDNControls.probe()))
    GM_registerMenuCommand('🌐 固定 CDN／恢復自動', showFixedCdnDialog)
    GM_registerMenuCommand('📝 開啟／關閉 verbose', () => {
        const result = BiliCDNControls.verbose(!Config.verbose)
        refreshPublicDiagnosticSnapshot()
        TrustedMenuUI.toast(result.message, 'success')
    })
    GM_registerMenuCommand('🧵 顯示 Worker 使用量', showWorkerStatsDialog)
    GM_registerMenuCommand('🧹 清除所有 soft block', () => {
        const result = BiliCDNControls.clearSoft()
        refreshPublicDiagnosticSnapshot()
        TrustedMenuUI.toast(result.message, result.status === 'empty' ? 'info' : 'success')
    })
    GM_registerMenuCommand('♻️ 救回單一 dead catalog 節點', showDeadReviveDialog)
    GM_registerMenuCommand('🗂️ 切換 catalog 自動選路節點', showCatalogDialog)
}
...
;(function () {
    ...
    const applyPageHooks = () => {
        ...
        const step = (name, fn) => {
            try { fn() } catch (e) { err('page hook 失敗（' + name + '）：', e) }
        }
    ...
    const setupSeekPrewarm = () => {
        ...
        // 舊版只從 activeCdnList[0..1] 取，而 activeCdnList 的 index
    ...
    const startRuntimeFeatures = () => {
        ...
        setupSeekPrewarm()
        // Keep-warm：瀏覽器 idle connection 約 30s 斷，搶在斷前刷新 catalog Top 3。
        if (!keepWarmTimer) {
    ...
    const buildUI = (settingsBar) => {
        ...
        const renderStatusHtml = (html) => {
            ...
            if (reportBtn) {
                reportBtn.addEventListener('click', (e) => {
                    if (!e || !e.isTrusted) return
                    e.stopPropagation()
                    copyDiagReport().then(result => {
                        reportBtn.textContent = result.ok ? '已複製診斷 ✓' : '請在視窗手動複製'
                        setTimeout(() => { if (document.contains(reportBtn)) reportBtn.textContent = '複製診斷' }, 1800)
                    }).catch(() => {})
                })
            }
        }
        ...
        const updateStatusPanel = () => {
            ...
            const selectableList = getHealthyCdnList().map(c => c.split('.')[0])
            ...
            let html = '<div style="color:#4fc3f7;">'
                + '候選順序：' + (activeList.length ? activeList.join(' > ') : '<span style="color:#ff7043;">無候選節點</span>')
                + '</div>'
                + '<div style="color:#80cbc4;font-size:9px;">真正可選：'
                + (selectableList.length ? selectableList.join(' > ') : '<span style="color:#ff7043;">無可用節點</span>')
                + '</div>'
                + '<div style="color:#80cbc4;font-size:9px;">非 catalog CDN 自動重導向 + Watchdog 自動切換</div>'
                + bufRow
