...
const VERSION = (typeof GM_info !== 'undefined' && GM_info.script && GM_info.script.version) || '1.5.5'
...
// XHR timeout 的期限由頁面呼叫端設定；原生事件只證明該期限已到，不能單獨授權 GM 健康
// 懲罰與主動測速。只有已由原生 progress 證明收到足量媒體資料、且請求實際持續至少 3 秒，
// 才把 timeout 視為可歸因的傳輸失敗。同 host 30 秒內只接受一次，避免並行請求同時到期
// 直接累積成持久黑名單。這不改寫播放器的 xhr.timeout，也不取消其請求。
...
const observeMediaTransfer = (context, url, bytes, source) => {
    ...
    if (kind === 'video' && mediaHeightMatches(rep)) {
        ...
        if (source === 'fetch' || source === 'xhr') {
            Watchdog.noteVideoTransport(lastVideoTransportObservation, observation)
    ...
    class XMLHttpRequest extends OriginalXMLHttpRequest {
        ...
        send(...args) {
            ...
            if (this._originCdn) {
                ...
                const active = () => !settled && self._biliRequestSeq === requestSeq && mediaContextActive(mediaContext)
                const listen = (type, callback) => {
                    const listener = e => {
                        if (e?.isTrusted !== true) return
                        if (!active()) { cleanup(); return }
                        try { callback(e) } catch { DiagnosticLog.fault('interceptor') }
                    }
                    listeners.push([type, listener]); self.addEventListener(type, listener)
                }
                listen('abort', () => { cleanup(); DiagnosticLog.updateRequest(diagnosticId, 'abort') })
                const fail = kind => {
                    cleanup()
                    DiagnosticLog.updateRequest(diagnosticId, 'network-error', { reason: kind, bytes: lastProgressLoaded })
                    handleVerifiedSegmentFailure({
                        ...
                        timeoutEvidence: kind === 'timeout' ? TRUSTED_XHR_TIMEOUT_EVIDENCE : null,
                ...
                listen('readystatechange', () => {
                    ...
                    cleanup()
                    DiagnosticLog.updateRequest(diagnosticId, self.status >= 400 ? 'http' : 'eof', {
            ...
            try { return super.send(...args) }
                ...
                mediaListenerCleanup.get(this)?.()
                DiagnosticLog.updateRequest(diagnosticRequests.get(this), 'network-error'); throw error
            }
        }
...
const handleVerifiedSegmentFailure = ({
    ...
    if (isTimeout && inSeekGrace()) return false
    if (isTimeout) {
        ...
        if (timeoutEvidence !== TRUSTED_XHR_TIMEOUT_EVIDENCE
            || bytes < MIN_THROUGHPUT_SAMPLE_BYTES || elapsed < XHR_TIMEOUT_MIN_ELAPSED_MS) {
            admission = 'insufficient'
        } else {
            const now = performance.now()
            for (const [host, acceptedAt] of acceptedXhrTimeoutAt) {
                if (!isValidCustomCdnHost(host) || !Number.isFinite(acceptedAt)
                    || now < acceptedAt || now - acceptedAt >= XHR_TIMEOUT_HOST_GAP_MS) {
                    acceptedXhrTimeoutAt.delete(host)
                }
            }
            if (acceptedXhrTimeoutAt.has(cdn)) admission = 'cooldown'
            else acceptedXhrTimeoutAt.set(cdn, now)
        }
        if (admission !== 'accepted') {
            DiagnosticLog.record('recovery', { host: cdn, kind: 'timeout', reason: admission,
    ...
    if (HARD_FAIL_STATUSES.has(numericStatus)) {
        ...
        if (!isTimeout) handleSegmentConnError(cdn, Math.max(0, publicFinite(bytesReceived)))
    } else {
        return false
    }

    addForcedRedirect(cdn)
    DiagnosticLog.record('recovery', { host: cdn, reason: isNetworkError || isTimeout ? kind : 'http', status: numericStatus,
...
const Watchdog = (() => {
    ...
    const penaltyState = host => ({ ...Object.fromEntries(penaltyFields.map(k => [k, cdnHealth[host]?.[k]])),
        until: cdnSoftBlockUntil[host] })
    const retractBurstPenalties = () => {
        ...
        records.reverse().forEach(({ host, before, after, at }) => {
            ...
            const current = penaltyState(host), h = cdnHealth[host]
            ...
            if (before.until === undefined) delete cdnSoftBlockUntil[host]
            else cdnSoftBlockUntil[host] = before.until
            hosts.add(host)
    ...
    const noteVideoTransport = (previous, observation) => {
        if (!freshMediaObservation(previous) || !observation.host || !previous.host || observation.host === previous.host) return
        observedSwitchCount = Math.min(Number.MAX_SAFE_INTEGER, observedSwitchCount + 1)
        DiagnosticLog.record('recovery', { reason: 'received', originalHost: previous.host, finalHost: observation.host })
    }
    ...
    const observeRecovery = (state, now) => {
        ...
        if (progress || r.count >= 3) {
            DiagnosticLog.record('recovery', { ...r, currentTime: state.currentTime, bufferAheadSec: state.bufferAheadSec,
                finalHost: getAttributedVideoHost(), outcome: progress ? 'progress' : 'no-progress' }, true)
            recoveryObservation = null
        }
    }
    const perCdnBytes     = {}
    ...
    const switchCdn = (reason) => {
        ...
        if (switchTimes.length >= SWITCH_BURST_MAX) {
            ...
            log('[Watchdog] ' + Math.round(SWITCH_BURST_WINDOW / 1000) + ' 秒內已嘗試修復 '
        ...
        recoveryObservation = { actionId: ++recoverySequence, generation: runtimeGeneration, epoch: playinfoEpoch,
            ...
            originalHost: getAttributedVideoHost() }
