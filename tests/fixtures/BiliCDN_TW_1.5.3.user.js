...
const interceptNetResponse = (function (theWindow) {
    ...
    const wrapMeasuredFetchResponse = (res, cdn, effectiveUrl, fetchStartedAt, failureContext, requestRuntimeToken, mediaContext) => {
    ...
    theWindow.fetch = (input, init) => {
        ...
        if (isMediaSegmentUrl(urlStr)) {
            ...
            return OriginalFetch(fetchInput, init).then(res => {
                ...
                if (res.ok) return wrapMeasuredFetchResponse(res, targetCdn, effectiveUrl, fetchStartedAt, failureContext, requestRuntimeToken, mediaContext)
