...
const rewriteUnstableMediaUrl = (urlStr) => {
    ...
    try {
        const u = parseMediaHttpUrl(urlStr)
        if (!u) return null
        if (!isMediaDeliveryPath(u.pathname)) return null
...
const replaceUrlHost = (urlStr, targetHost) => {
    ...
    // 直播使用獨立 CDN tier，點播 catalog 無法服務 /live-bvc/。
    if (isLiveBvcUrl(urlStr)) {
        redirectStats.liveSkipped = Math.min(10000, (redirectStats.liveSkipped || 0) + 1)
        return null
    }
...
const isBiliFragmentUrl = (url) => {
    ...
    try {
        ...
        return MEDIA_PATH_RE.test(path) || path.includes('/upgcxcode/') || path.startsWith('/v1/resource')
    } catch {
...
const biliCdnClassify = (url) => {
    try {
        const u = new URL(url);
        const host = u.hostname.toLowerCase();
        const path = u.pathname;
        if (path.includes('/live-bvc/')) return 'live';
        const media = /\\.(m4s|mp4|flv|m3u8)$/i.test(path) || path.includes('/upgcxcode/') || path.startsWith('/v1/resource');
        if (!media) return 'unknown';
        const nonDefaultPort = !!u.port && u.port !== '80' && u.port !== '443';
        const ipPort = /^(?:\\d{1,3}\\.){3}\\d{1,3}$/.test(host) && nonDefaultPort;
        if (biliCdnIsUnstable(host) || String(u.searchParams.get('os') || '').toLowerCase() === 'mcdn' || ipPort) return 'pcdn';
        return nonDefaultPort ? 'suspected-pcdn' : 'normal';
...
const biliCdnRewrite = (url) => {
    try {
        ...
        if (verdict === 'live' || verdict === 'suspected-pcdn') return absolute;
...
const probeCdnThroughput = (cdn, sampleUrl, probeBytes, externalSignal) => new Promise((resolve) => {
    ...
    let timedOut = false
    const to   = scheduleRuntimeTimeout(() => {
        timedOut = true
    ...
    const finishPartialTimeout = () => {
        if (!timedOut || settled || bytes < THRPT_PROBE_MIN_BYTES
            || disabled || (externalSignal && externalSignal.aborted)) return false
        const dl = Math.max(1, performance.now() - t0 - ttfb)
        recordCdnThroughput(cdn, bytes, dl, playbackRateState.effectiveRate)
        recordCdnLatency(cdn, Math.max(1, ttfb))
        redirectStats.partialProbeSamples = Math.min(10000, (redirectStats.partialProbeSamples || 0) + 1)
        done({ cdn, partial: true })
        return true
    }
    ...
    interceptNetResponse.rawFetch(target, {
        ...
        const pump = () => reader.read().then(({ done: rdone, value }) => {
            ...
            pump()
        }).catch(() => { if (!finishPartialTimeout()) done(null) })
        pump()
    }).catch(() => { if (!finishPartialTimeout()) done(null) })
})
    ...
    return TrustedMenuUI.openActions({
        ...
        paragraphs: [
            ...
            '倍速：' + playbackRateState.effectiveRate + 'x（' + (playbackRateState.confirmed ? '已確認' : '假定')
                + '）｜buffer：' + stats.bufferAheadSec + 's / ' + stats.targetMB + 'MB',
...
;(function () {
    ...
    const buildUI = (settingsBar) => {
        ...
        const updateStatusPanel = () => {
            ...
            const rate = playbackRateState.effectiveRate + 'x' + (playbackRateState.confirmed ? '' : '（假定）')
            const softCount = Object.keys(cdnSoftBlockUntil).filter(isCdnSoftBlocked).length
            ...
            let html = '<div style="color:#4fc3f7;">'
                ...
                + (s.reachedTarget ? ' <span style="color:#66bb6a;">✓</span>' : '')
                + '</div>'
