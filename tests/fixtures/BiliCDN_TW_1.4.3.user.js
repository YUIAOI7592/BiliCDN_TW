...
const VERSION = (typeof GM_info !== 'undefined' && GM_info.script && GM_info.script.version) || '1.4.3'
...
// 在 document-start 的 sandbox 內先捕捉對話框入口；只有 Tampermonkey menu callback 會呼叫，
// 回傳值仍視為不可信並以 catalog 編號重新解析。
const trustedPrompt = typeof prompt === 'function' ? prompt.bind(window) : null
...
const interceptNetResponse = (function (theWindow) {
    ...
    class XMLHttpRequest extends OriginalXMLHttpRequest {
        ...
        send(...args) {
            ...
            if (this._originCdn) {
                ...
                this.addEventListener('abort', () => { if (self._biliRequestSeq === requestSeq) aborted = true }, { once: true })
    ...
    theWindow.fetch = (input, init) => {
        ...
        return OriginalFetch(input, init).then(async response => {
            ...
            return cloneResponseWithBody(response, handleInterceptedResponse(text, urlStr), false)
        })
    }
...
const Watchdog = (() => {
    ...
    const onEntry = (entry) => {
        if (disabled || !entry || !entry.name) return
...
// 剪貼簿 API 仍可能因權限或瀏覽器策略失敗；失敗時退回可信選單開啟的 prompt，
// 讓使用者手動選取複製。
...
const BiliCDNControls = {
    ...
    bakeoff() {
        ...
        return runThroughputBakeoff(lastSampleSegmentUrl, false, trustedBakeoffRequest('menu')).then(() => {
        ...
        return reorderCdnsByLatency(true).then(() => {
            ...
            if (skipped.length) {
                ...
                console.log('[BiliCDN] 要翻案只有一個有效作法——讓它真的去服務 segment：'
                    + '請由 Tampermonkey「固定 CDN／恢復自動」選單固定使用它並播一段影片；'
                    + '成功後推定會自動失效，再由同一選單回到自動選路。')
            }
            return this.diag()
        })
    },
...
const promptForTrustedCdn = () => {
    if (!trustedPrompt) {
        console.error('[BiliCDN] 此環境無法顯示固定 CDN 選擇框')
        return
    }
    const choices = ['0. 自動選路', ...TRUSTED_CDN_CATALOG.map((host, index) => (index + 1) + '. ' + host)]
    const answer = trustedPrompt('[BiliCDN] 選擇固定 CDN：\n' + choices.join('\n'), '0')
    if (answer == null) return
    const normalized = String(answer).trim()
    if (!/^\d+$/.test(normalized)) {
        console.error('[BiliCDN] 無效選項，設定未變更')
        return
    }
    ...
    BiliCDNControls.setCdn(index === 0 ? null : TRUSTED_CDN_CATALOG[index - 1])
    refreshPublicDiagnosticSnapshot()
    try { location.reload() } catch {}
}
const promptForDeadRevive = () => {
    if (!trustedPrompt) return
    const dead = listDeadHosts().map(entry => entry.host).filter(host => TRUSTED_CDN_CATALOG_SET.has(host))
    if (!dead.length) {
        console.log('[BiliCDN] 目前沒有可救回的 catalog 死節點')
        return
    }
    const answer = trustedPrompt('[BiliCDN] 選擇要救回的節點：\n'
        + dead.map((host, index) => (index + 1) + '. ' + host).join('\n'), '1')
    if (answer == null || !/^\d+$/.test(String(answer).trim())) return
    const index = Number(String(answer).trim()) - 1
    if (!Number.isInteger(index) || index < 0 || index >= dead.length) return
    reviveDeadHost(dead[index])
    promoteBestCdnNow()
    syncWorkerCdnTarget()
    refreshPublicDiagnosticSnapshot()
}
const promptForCatalogOverride = () => {
    if (!trustedPrompt) return
    const lines = TRUSTED_CDN_CATALOG.map((host, index) =>
        (index + 1) + '. [' + (isCatalogAutoEnabled(host) ? '啟用' : '停用') + '] ' + host)
    const answer = trustedPrompt('[BiliCDN] 切換自動選路 catalog 節點：\n0. 清除 override、恢復檔頭預設\n'
        + lines.join('\n'), '0')
    if (answer == null || !/^\d+$/.test(String(answer).trim())) return
    const index = Number(String(answer).trim())
    if (!Number.isInteger(index) || index < 0 || index > TRUSTED_CDN_CATALOG.length) return
    if (index === 0) resetCatalogOverrides()
    else {
        const host = TRUSTED_CDN_CATALOG[index - 1]
        setCatalogOverride(host, !isCatalogAutoEnabled(host))
    }
}
if (typeof GM_registerMenuCommand === 'function') {
    GM_registerMenuCommand('🔄 重置所有學習狀態', () => {
        BiliCDNControls.reset()
        refreshPublicDiagnosticSnapshot()
        try { location.reload() } catch {}
    })
    GM_registerMenuCommand('📊 顯示診斷資訊', () => {
        const result = BiliCDNControls.diag()
        refreshPublicDiagnosticSnapshot()
        return result
    })
    GM_registerMenuCommand('📋 複製診斷報告', () => BiliCDNControls.report())
    GM_registerMenuCommand('🏁 立即測速選節點', () => BiliCDNControls.bakeoff())
    GM_registerMenuCommand('📡 手動延遲探測', () => BiliCDNControls.probe())
    GM_registerMenuCommand('🌐 固定 CDN／恢復自動', promptForTrustedCdn)
    GM_registerMenuCommand('📝 開啟／關閉 verbose', () => {
        Config.verbose = !Config.verbose
        GM_setValue('verbose', Config.verbose)
        refreshPublicDiagnosticSnapshot()
        console.log('[BiliCDN] verbose：' + (Config.verbose ? '開啟' : '關閉'))
    })
    GM_registerMenuCommand('🧵 顯示 Worker 使用量', () => BiliCDNControls.workerStats())
    GM_registerMenuCommand('🧹 清除所有 soft block', () => BiliCDNControls.clearSoft())
    GM_registerMenuCommand('♻️ 救回單一 dead catalog 節點', promptForDeadRevive)
    GM_registerMenuCommand('🗂️ 切換 catalog 自動選路節點', promptForCatalogOverride)
}
    ...
    const transformInitialPlayInfo = () => {
        ...
        if (unsafeWindow.__playinfo__) {
            ...
            Object.defineProperty(unsafeWindow, '__playinfo__', {
                get: () => internal,
                set: v => { if (!disabled) playInfoTransformer(v); internal = v },
                configurable: true
            })
        }
    }
