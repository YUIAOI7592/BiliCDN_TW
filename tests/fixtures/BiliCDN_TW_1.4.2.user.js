...
// BiliCDN.workerStats()：mediaSeen > 0 才代表 Worker 真的抓過影片分段、值得保留。
...
const VERSION = (typeof GM_info !== 'undefined' && GM_info.script && GM_info.script.version) || '1.4.2'
...
const rebuildPreferredCdnList = () => {
    const next = PREFERRED_CDN_LIST_RAW.filter(h => !matchesExclude(h))
    ...
    try {
        ...
        Object.entries(raw).forEach(([cdn, h]) => {
            ...
            out[cdn] = {
                ...
                samples:  hasCurrentThroughput ? Math.min(+h.samples || 0, 12) : 0,
                bytes: +h.bytes || 0,
                failures: Math.min(+h.failures || 0, 2),
                successes: Math.min(+h.successes || 0, 12),
                slowSamples: hasCurrentThroughput ? Math.min(+h.slowSamples || 0, 3) : 0,
        ...
        try {
            ...
            allCdns.forEach(cdn => {
                ...
                payload[cdn] = {
                    ...
                    samples: hasThroughput ? (+throughput.samples || 0) : 0,
                    ...
                    slowSamples: hasThroughput ? (+throughput.slowSamples || 0) : 0,
                    probeTimeouts: +state.probeTimeouts || 0,
                    probeSlows: +state.probeSlows || 0,
...
const interceptNetResponse = (function (theWindow) {
        ...
        send(...args) {
            ...
            if (this._originCdn) {
                ...
                this.addEventListener('error', () => {
                    if (self._biliRequestSeq !== requestSeq || aborted) return
                ...
                this.addEventListener('readystatechange', function () {
                    if (self._biliRequestSeq !== requestSeq) return
    ...
    const wrapMeasuredFetchResponse = (res, cdn, effectiveUrl, fetchStartedAt, failureContext) => {
    ...
    theWindow.fetch = (input, init) => {
        ...
        if (isMediaSegmentUrl(urlStr)) {
            ...
            return OriginalFetch(fetchInput, init).then(res => {
                ...
                if (res.ok) return wrapMeasuredFetchResponse(res, targetCdn, effectiveUrl, fetchStartedAt, failureContext)
...
const buildDiagReport = () => {
    ...
    const lines = [
        ...
        'Codec 偏好：' + resolvedVideoCodecPreference + '；最近排序=' + JSON.stringify(lastCodecDecision.groups),
