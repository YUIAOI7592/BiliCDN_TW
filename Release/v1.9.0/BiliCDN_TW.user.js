...
(() => {
  ...
  function createSettings(deps, input) {
    ...
    const VERSION = typeof GM_info !== "undefined" && GM_info.script && GM_info.script.version || "1.9.0";
  ...
  function createNativeRoutes(deps) {
    ...
    const resolveRequestRouteUnchecked = /* @__PURE__ */ __name((url, context) => {
      ...
      if (exactSignedRoute && group.source !== "player-mpd" && !plannedAffinity?.enforced && !avoidedHosts.has(exactSignedRoute.host) && plannedAffinity?.type !== "native-signed" && (!plannedAffinity || exactSignedRoute.host !== plannedAffinity.host)) {
        return {
  ...
  function createPlayerManifest(deps) {
    ...
    return { start: start2, cancel, supersede, reconcileNow, reconcileMediaRequest, diagnostics };
  }
