...
(() => {
  ...
  var SignedRouteVault = class {
    ...
    register(input) {
      ...
      for (const raw of [...new Set(input.urls)].slice(0, MAX_URLS_PER_GROUP)) {
        ...
        const identity2 = mediaIdentity(parsed.url.href);
        if (identity2) this.#byIdentity.set(identity2, rep);
  ...
  var RouteCoordinator = class {
    ...
    async recordChallenge(applied, bytes, elapsedMs, ttfbMs, outcome, failureKind) {
      ...
      await this.evidence.record(applied.decision.host, context.kind, {
        ...
        source: "challenge",
        outcome,
        throughputMbps: outcome === "success" && bytes >= 64 * 1024 && elapsedMs > 0 ? bytes * 8 / elapsedMs / 1e3 : null,
        ttfbMs,
        failureKind
      });
    }
    async observe(observation) {
      ...
      if (observation.status === 403 && observation.streamKey && observation.routeType === "catalog-generated" && observation.originalHost !== observation.targetHost) {
        ...
        if (observation.kind === "video" && record) {
          ...
          if (record.representation && record.representation !== active) {
            if (this.#tentativeRepresentation === record.representation) this.#tentativeTransfers++;
              ...
              this.#tentativeRepresentation = record.representation;
          ...
          if (record.representation && this.session.get().representation === record.representation) {
            this.session.setAffinity({
              ...
              decisionId: observation.decisionId ?? record.decision.id,
    ...
    #choose(context, demand, boundary, failedHost) {
      ...
      const currentRoute = current && current.action !== "block" && current.host ? { type: current.routeType, host: current.host } : affinity ? { type: affinity.type, host: affinity.host } : null;
  ...
  var MeasurementController = class {
    ...
    async #run(applied, demand, controller, marker) {
      ...
      let bytes = 0, responseAt = 0, failure = null, ok = false;
      try {
  ...
  var PlayurlAdapter = class {
    ...
    transform(payload, source = "trusted-api") {
      ...
      for (const [kind, items] of [["video", dash.video], ["audio", dash.audio]]) {
        items.forEach((item, index) => {
          ...
          const requiredMbps = Math.max(
            kind === "audio" ? 0.5 : 2,
            (bandwidth || (kind === "audio" ? 192e3 : 4e6)) / 1e6 * 2 * 1.25
          );
          ...
          rewriteItem(item, applied, [.../* @__PURE__ */ new Set([...catalogBackups, ...urls])].filter((url) => url !== applied).slice(0, 5));
        });
      }
      return true;
    }
  ...
  var PlayerAdapter = class {
    ...
    playbackRate() {
      return safeNumber(safeCall(this.player(), "getPlaybackRate")) ?? this.snapshot().playbackRate;
    }
    seek(value) {
    ...
    play() {
      return safeCall(this.player(), "play");
    }
    reset() {
  ...
  var mediaKind = /* @__PURE__ */ __name((applied) => applied.context?.kind ?? null, "mediaKind");
  ...
  var TransportAdapter = class {
    ...
    #installFetch() {
      ...
      const wrapped = /* @__PURE__ */ __name(async function(input, init) {
        ...
        if (isPlayurlApi(originalUrl)) {
          ...
          return copyResponseSurface(new Response(text2, { status: response2.status, statusText: response2.statusText, headers: response2.headers }), response2);
        }
        ...
        try {
          ...
          void self.#observeFetch(applied, originalUrl, applied.url, null, startedAt, 0, 0, aborted ? "abort" : "failure", aborted ? void 0 : "network");
          throw error;
        }
    ...
    #installXhr() {
      ...
      const send = /* @__PURE__ */ __name(function(body) {
        ...
        if (!meta || self.settings.get().disabled) {
          ...
          Object.defineProperty(proto, "response", { ...responseDescriptor, get() {
            ...
            if (this.responseType === "json" && raw && typeof raw === "object") {
              if (meta.transformedJson === void 0) {
                self.playurl.transform(raw, "trusted-api");
    ...
    async #observeFetch(applied, originalUrl, targetUrl, response, startedAt, responseAt, bytes, outcome, failureKind) {
    ...
    #observation(applied, originalUrl, targetUrl, finalUrl, status, bytes, startedAt, responseAt, outcome, failureKind) {
      ...
      return {
        ...
        kind,
        routeType,
        originalHost: applied.sourceHost ?? hostOf(originalUrl),
  ...
  var CONTEXT_MS = 6e4;
  var INCIDENT_AFTER_MS = 3e4;
  var INCIDENT_MAX_MS = 9e4;
  var MAX_EVENTS_BYTES = 64 * 1024;
  var MAX_REPORT_BYTES = 96 * 1024;
  var boundedText = /* @__PURE__ */ __name((value, max = 64) => String(value ?? "").slice(0, max), "boundedText");
  var safeDecision = /* @__PURE__ */ __name((decision, safeHost3) => ({
    id: decision.id,
    action: decision.action,
    reason: decision.reason,
    routeType: decision.routeType,
    host: safeHost3(decision.host),
    ranking: decision.ranking.slice(0, 12).map((row) => ({
      host: safeHost3(row.candidate.host),
      type: row.candidate.type,
      state: row.state,
      eligible: row.eligible,
      reasons: row.reasons,
  ...
  var safeTransport = /* @__PURE__ */ __name((observation, safeHost3) => ({
    decisionId: observation.decisionId,
    representation: observation.representation,
    kind: observation.kind,
    routeType: observation.routeType,
    originalHost: safeHost3(observation.originalHost),
    targetHost: safeHost3(observation.targetHost),
    finalHost: safeHost3(observation.finalHost),
  ...
  var DiagnosticRecorder = class {
    ...
    #incidentReplaced = 0;
    #evicted = 0;
    #externalAliases = /* @__PURE__ */ new Map();
    record(event) {
      this.#freezeIfDue();
      ...
      if (this.#incident?.state === "capturing") {
        ...
        this.#incident.captureUntil = Math.min(
          this.#incident.startedAt + INCIDENT_MAX_MS,
    ...
    mark(reason = "manual") {
      this.#startIncident(boundedText(reason), this.now());
    }
    clear() {
      this.#incident = null;
    }
    snapshot() {
      ...
      return Object.freeze({
        ...
        incident: this.#incident ? {
          ...
          captureUntil: this.#incident.captureUntil,
          events: Object.freeze(this.#incident.events.map((event) => ({ ...event, data: { ...event.data } })))
        } : null,
    ...
    buildReport(readModel) {
      ...
      let text2 = JSON.stringify(payload, null, 2);
      if (new TextEncoder().encode(text2).byteLength > MAX_REPORT_BYTES) {
        text2 = JSON.stringify({ ...payload, recorder: { ...recorder, events: [], flow: [], truncated: true } }, null, 2);
      }
      if (new TextEncoder().encode(text2).byteLength > MAX_REPORT_BYTES) {
        const current = this.#essentialReadModel(readModel);
        const incident = recorder["incident"];
        for (const limit of [32, 16, 8, 4, 1, 0]) {
          text2 = JSON.stringify({ ...payload, current, recorder: {
            incident: incident ? { ...incident, events: limit === 0 ? [] : incident.events?.slice(-limit) ?? [] } : null,
            counters: recorder["counters"],
            truncated: true
          } }, null, 2);
          if (new TextEncoder().encode(text2).byteLength <= MAX_REPORT_BYTES) break;
        }
      }
      if (new TextEncoder().encode(text2).byteLength > MAX_REPORT_BYTES) {
        text2 = JSON.stringify({
          ...payload,
          current: this.#essentialReadModel(readModel),
          recorder: {
            incident: this.#incident ? {
              id: this.#incident.id,
              reason: this.#incident.reason,
    ...
    #aggregate(observation) {
      const bucket = Math.floor(observation.completedAt / 5e3), key = `${bucket}:${observation.kind ?? "unknown"}:${observation.routeType}:${this.#safeHost(observation.targetHost)}:${this.#safeHost(observation.finalHost) ?? ""}`;
      ...
      row.lastAt = observation.completedAt;
      row.maxElapsedMs = Math.max(row.maxElapsedMs, observation.elapsedMs);
      row.maxTtfbMs = Math.max(row.maxTtfbMs, observation.ttfbMs ?? 0);
      if (observation.outcome === "success") row.successes++;
      else if (observation.outcome === "abort") row.aborts++;
      else row.failures++;
      this.#flow.set(key, row);
      while (this.#flow.size > 96) this.#flow.delete(this.#flow.keys().next().value);
    }
    #safeEvent(event) {
      switch (event.type) {
        case "route-decision":
          return Object.freeze({ at: event.at, type: event.type, data: Object.freeze(safeDecision(event.decision, (host) => this.#safeHost(host))) });
        case "transport":
          return Object.freeze({ at: event.at, type: event.type, data: Object.freeze(safeTransport(event.observation, (host) => this.#safeHost(host))) });
        case "route-observed":
          return Object.freeze({ at: event.at, type: event.type, data: Object.freeze({ routeType: event.routeType, host: this.#safeHost(event.host), decisionId: event.decisionId }) });
        case "recovery":
          return Object.freeze({ at: event.at, type: event.type, data: Object.freeze({
            action: event.action.action,
            id: event.action.id,
            ...event.action.action === "route-fallback" ? { kind: event.action.kind, decision: safeDecision(event.action.decision, (host) => this.#safeHost(host)) } : event.action.action === "player-reload" ? { savedPositionSec: event.action.savedPositionSec, savedRate: event.action.savedRate } : { reason: event.action.reason }
          }) });
        case "core":
          return Object.freeze({ at: event.at, type: event.type, data: Object.freeze({ state: event.state, actionId: event.actionId }) });
        case "lifecycle":
          return Object.freeze({ at: event.at, type: event.type, data: Object.freeze({ generation: event.generation, epoch: event.epoch, reason: boundedText(event.reason) }) });
      }
    }
    #triggerReason(event) {
    ...
    #startIncident(reason, at) {
      ...
      this.#incident = {
        id: `incident-${++this.#incidentSerial}`,
        reason,
        startedAt: at,
        captureUntil: at + INCIDENT_AFTER_MS,
        state: "capturing",
        events: this.#events.filter((event) => at - event.at <= CONTEXT_MS)
      };
    }
    #freezeIfDue() {
    ...
    #prune() {
      ...
      while (JSON.stringify(this.#events).length > MAX_EVENTS_BYTES && this.#events.length) {
        this.#events.shift();
        this.#evicted++;
      }
    }
    ...
    #essentialReadModel(readModel) {
      const pick = /* @__PURE__ */ __name((value, names) => {
        if (!value || typeof value !== "object") return {};
        const source = value, result = {};
        for (const name of names) if (name in source) result[name] = source[name];
        return result;
      }, "pick");
      const routes = readModel["routes"];
      return this.#sanitize({
        version: readModel["version"],
        session: pick(readModel["session"], ["generation", "epoch", "representation", "affinity", "disabled", "recovering"]),
        ...
        evidence: Array.isArray(readModel["evidence"]) ? readModel["evidence"].slice(0, 8) : [],
        truncated: true
      });
    }
    #sanitize(value, key = "", depth = 0) {
      ...
      const output = {};
      for (const [name, item] of Object.entries(value).slice(0, 128)) output[name.slice(0, 64)] = this.#sanitize(item, name, depth + 1);
      return output;
    }
  };
...
核心：${this.deps.recovery.snapshot().state}｜單一挑戰者：${this.deps.measurement.snapshot().state}`;
    ...
    #readModel() {
      ...
      return Object.freeze({
        version: GM_info?.script?.version ?? "2.0.0",
    ...
    #row(label, control, detail = "") {
      ...
      if (detail) {
        ...
        text2.append(document.createElement("br"), small);
      }
