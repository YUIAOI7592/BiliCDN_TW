...
(() => {
  ...
  var SignedRouteVault = class {
    ...
    register(input) {
      ...
      const routes = promoted ? [] : [...prior?.routes ?? []];
      for (const raw of [...new Set(input.urls)].slice(0, MAX_URLS_PER_GROUP)) {
        const parsed = parseMediaUrl(raw);
        ...
        routes.push(Object.freeze({
          ...
          url: parsed.url.href,
          order: routes.length,
          activelyExplorable: !opaque && isKnownNativeFamily(parsed.host),
          selectable: !opaque
        }));
      ...
      const identity = !promoted && prior ? prior.identity : Object.freeze({
    ...
    registerOutput(rep, original, primary, backups, decisionId2, source) {
  ...
  var RouteCoordinator = class {
    ...
    startupOptions(url, catalogCursor = 0) {
      ...
      if (!parsed || parsed.kind !== "normal" || !parsed.replaceable || !context || this.settings.get().fixedHost || this.#originalComparison || this.session.get().disabled) return null;
      ...
      const add = /* @__PURE__ */ __name((host, type, candidateUrl, original) => {
        if (candidates.length >= 3 || candidates.some((candidate) => candidate.host === host) || this.#hardRestriction(host, context.kind) || this.vault.isInvalid(context.representation, host)) return;
    ...
    commitStartupChoice(url, selected, reason) {
      ...
      if (selected.original) decision = this.#pass(reason, selected.host, context.kind);
        ...
        if (catalogIndex2 < 0 || replaceUrlHost(url, selected.host) !== selected.url) return null;
    ...
    async recordStartupSuccess(candidate, bytes2, elapsedMs, ttfbMs) {
      ...
      const valid = /* @__PURE__ */ __name(() => context.generation === this.session.get().generation && context.epoch === this.session.get().epoch && !this.session.get().disabled && this.vault.isCurrentIdentity(context), "valid");
      if (!valid()) return;
      await this.evidence.record(candidate.host, context.kind, {
        requestId: `startup:${++this.#serial}:${candidate.host}`,
        at: this.clock.now(),
        source: "challenge",
        outcome: "success",
    ...
    playerOutput(representation, original, decision, originals, source = "trusted-api") {
      const context = this.vault.identity(representation);
      if (!context) return { primary: "", backups: [] };
      ...
      if (this.#originalComparison) {
        ...
        const primary2 = permitted[0] ?? "", backups2 = permitted.slice(1, 6);
        if (primary2) {
        ...
        return { primary: primary2, backups: backups2 };
      }
      ...
      const distinct = [], sameHost = [];
      const append = /* @__PURE__ */ __name((url) => {
        const host = parseMediaUrl(url)?.host;
        if (!host || url === primary || !allowed(url)) return;
        if (usedHosts.has(host)) {
          if (!sameHost.includes(url)) sameHost.push(url);
          return;
        }
        usedHosts.add(host);
        distinct.push(url);
    ...
    #choose(context, demand, boundary, failedHost) {
      ...
      const catalog = TRUSTED_CATALOG.map((host, index) => Object.freeze({
        ...
        kind: context.kind,
        catalogIndex: index
      })).filter((candidate) => !this.vault.isInvalid(context.representation, candidate.host) && !this.#incompatible(root, candidate.host));
    ...
    #catalogOnly(demand, originalHost, knownKind) {
  ...
  var TIMEOUT_MS = 3e3;
  var directResponseFrom = /* @__PURE__ */ __name((response, host) => {
  ...
  var MeasurementController = class {
    ...
    async #runStartup(url, options, controller, marker) {
      ...
      let winner = ranked[0]?.candidate ?? options.candidates.find((candidate) => candidate.original) ?? [...valid].sort((a, b) => (b.safeMbps ?? 0) - (a.safeMbps ?? 0))[0]?.candidate ?? options.candidates[0] ?? null;
    ...
    async #probeStartup(candidate, demand, signal) {
      ...
      let bytes2 = 0, responseAt = 0;
      try {
        const response = await this.nativeFetch(candidate.url, {
          method: "GET",
        ...
        try {
          while (bytes2 < limit && !signal.aborted) {
            const item = await reader.read();
            if (item.done) break;
            bytes2 = Math.min(limit, bytes2 + item.value.byteLength);
            if (bytes2 >= limit) {
              await reader.cancel("startup-sample-complete");
              break;
            }
          }
        } finally {
        ...
        const valid = candidate.cachedSafeMbps !== null ? bytes2 > 0 : bytes2 >= 64 * 1024;
        const elapsedMs = Math.max(1, this.now() - startedAt);
        return {
          candidate,
          valid,
          safeMbps: valid ? candidate.cachedSafeMbps ?? bytes2 * 8 / elapsedMs / 1e3 * 0.7 : null,
          bytes: bytes2,
          elapsedMs,
          ttfbMs: responseAt - startedAt,
          status: response.status,
          reason: valid ? "measured" : "short-response"
        };
      } catch {
        return {
          candidate,
          ...
          bytes: bytes2,
          elapsedMs: Math.max(1, this.now() - startedAt),
          ttfbMs: responseAt ? responseAt - startedAt : null,
          status: null,
    ...
    async #tryStart(status) {
      ...
      try {
        ...
        await this.storage.withLock("measurement", () => {
          ...
          first = this.routes.challenge(status.representation, status.demand, this.#preferNative, /* @__PURE__ */ new Set(), cursor);
          if (!first?.url) return;
    ...
    async #runCandidate(applied, demand, round, marker) {
      ...
      let bytes2 = 0, responseAt = 0, failure = null, ok = false, reason = "http-status";
      try {
      ...
      await this.routes.recordChallenge(
        applied,
        bytes2,
        elapsed,
        responseAt ? responseAt - startedAt : null,
        ok ? "success" : "failure",
        failure
      );
      this.#snapshot = Object.freeze({
        state: ok ? "complete" : "failed",
        reason: ok ? "sample-recorded" : reason,
  ...
  var RecoveryController = class {
    ...
    tick(snapshot) {
      ...
      if (!token.reloadingAt && (dead || startupStalled) && now - token.startedAt >= 4e3) this.#reload(token);
  ...
  var PlayerMonitor = class {
    ...
    tick() {
      ...
      if (!disabled && !originalMode && watchdog === "recovering" && (this.#startupObservedProgress || !firstMediaAt) && now - this.#lastRecoveryAt >= 3e4) {
      ...
      for (const [kind, items] of [["video", dash.video], ["audio", dash.audio]]) {
        items.forEach((item, index) => {
          ...
          if (parseMediaUrl(primary)?.kind === "unknown") {
            ...
            if (rep2 && source !== "trusted-api" && this.vault.source(rep2) === "trusted-api") return;
            if (source !== "player-mpd") {
              const output2 = rep2 ? this.routes.opaqueOutput(rep2, primary, urls, source) : { primary: "", backups: [] };
              rewriteItem(item, output2.primary, output2.backups);
            }
            return;
          }
          ...
          if (!rep) {
            if (source !== "player-mpd") {
              ...
              const backups = primaryOutput.url ? [...new Set(urls.map((url) => this.routes.apply(url).url).filter((url) => !!url && url !== primaryOutput.url))].slice(0, 5) : [];
  ...
  var TransportAdapter = class {
    constructor(session, settings, routes, playurl, measurement, now) {
      this.session = session;
      this.settings = settings;
      this.routes = routes;
      this.playurl = playurl;
    ...
    snapshot() {
      ...
      try {
        fetchInstalled = this.#installed && unsafeWindow.fetch === this.#fetchRef;
      } catch {
      }
      try {
        xhrInstalled = this.#installed && unsafeWindow.XMLHttpRequest?.prototype.open === this.#xhrOpenRef && unsafeWindow.XMLHttpRequest?.prototype.send === this.#xhrSendRef;
      } catch {
      }
      return Object.freeze({
        hookState: this.#installed && (!fetchInstalled || !xhrInstalled) ? "degraded" : this.#hookState,
    ...
    dispose() {
      ...
      this.#fetchRef = null;
      this.#xhrOpenRef = null;
      this.#xhrSendRef = null;
    }
    #installFetch() {
      ...
      const wrapped = /* @__PURE__ */ __name(async function(input, init) {
        ...
        if (isPlayurlApi(originalUrl)) {
          const generation2 = self.session.get().generation, responseKey = `api-fetch-${++self.#requestSerial}`;
          ...
          try {
            ...
            if (self.session.isGeneration(generation2) && !self.settings.get().disabled) self.playurl.transform(payload, "trusted-api", responseKey);
        ...
        if (!self.session.isGeneration(generation)) {
          ...
          if (original2.decision.action === "block" || !original2.url) {
            self.#stats.blocked++;
            self.#lastBlocked = Object.freeze({ method, host: hostOf(originalUrl), reason: original2.decision.reason });
        ...
        if (applied.decision.action === "block" || !applied.url) {
          self.#stats.blocked++;
          self.#lastBlocked = Object.freeze({ method, host: hostOf(originalUrl), reason: applied.decision.reason });
    ...
    #installXhr() {
      ...
      const open = /* @__PURE__ */ __name(function(method, url, async = true, username, password) {
        ...
        if (previous) {
          previous.abortedBeforeSend = true;
          previous.cleanup();
        }
      ...
      try {
        ...
        this.#xhrOpenRef = open;
        this.#xhrSendRef = send;
  ...
  var DiagnosticRecorder = class {
    ...
    buildReport(readModel) {
      ...
      recorder.rankings = [];
      output = JSON.stringify(payload);
      const events = recorder.events, flows = recorder.flow, incident = recorder.incident;
      ...
      const summary = {
        ...
        current: this.#sanitize({ version: m.version, monitor: { watchdog: m.monitor?.watchdog }, truncated: true }),
        recorder: {
...
`;
  ...
  var ControlCenter = class {
    ...
    #renderOverview() {
      ...
      const mode = this.deps.routes.isOriginalComparison() ? "本分頁只用 B 站提供的網址（對照測試）" : settings.fixedHost ? `固定使用 ${settings.fixedHost}` : "自動挑選 CDN";
...
${label}備用路線：${row.plannedHost}｜${stage}` + (row.responseHost ? `｜回應來自 ${row.responseHost}${row.status ? `（HTTP ${row.status}）` : ""}` : "");
      ...
      const blacklistButton = this.#button("封鎖最近成功回應的影片 CDN 24 小時（影片＋音訊）", async () => {
        ...
        if (host) await this.deps.restrictions.add({ host, type: "black", kind: "all", reason: "user", expireAt: this.deps.now() + 24 * 60 * 60 * 1e3 });
    ...
    #renderSettings() {
      ...
      rows.append(this.#toggle(
        ...
        (value) => this.deps.settings.update({ blockWebRtc: value }),
        "避免網頁建立點對點連線；不熟悉這項功能時，可維持預設值。"
      ));
      rows.append(this.#toggle(
        "阻止網頁使用 HTTPDNS",
        settings.blockHttpDns,
        (value) => this.deps.settings.update({ blockHttpDns: value }),
        "避免網頁透過 HTTPDNS 另行尋找 CDN；不熟悉這項功能時，可維持預設值。"
      ));
      rows.append(this.#toggle(
        "記錄更多診斷細節",
        settings.verbose,
        (value) => this.deps.settings.update({ verbose: value }),
  ...
  var start = /* @__PURE__ */ __name(() => {
    ...
    const diagnosticSample = /* @__PURE__ */ __name((video, watchdog, at) => ({
      at,
      generation: session.get().generation,
      epoch: session.get().epoch,
      enabled: !settings.get().disabled,
      originalComparison: routes.isOriginalComparison(),
