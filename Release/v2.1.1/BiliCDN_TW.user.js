...
(() => {
  ...
  var SignedRouteVault = class {
    ...
    registerOutput(rep, original, primary, backups) {
  ...
  var RouteCoordinator = class {
    ...
    recoverStartup(representation, demand, failedHost, preferredHosts) {
      ...
      if (decision && decision.host !== failedHost) this.#emit({ type: "recovery", at: this.clock.now(), action: {
        action: "route-fallback",
        id: recoveryActionId(`recovery-${++this.#recoverySerial}`),
        kind: "video",
        decision
    ...
    playerOutput(representation, original, decision, originals) {
      const context = this.vault.identity(representation);
      ...
      const generated = locked && this.#hostLockedStreams.has(locked) ? [] : decision.ranking.filter((row) => row.eligible && row.candidate.type === "catalog-generated").flatMap((row) => {
    ...
    #choose(context, demand, boundary, failedHost) {
      ...
      const catalog = TRUSTED_CATALOG.map((host, index) => Object.freeze({
        ...
        kind: context.kind,
        catalogIndex: index
      })).filter((candidate) => !this.vault.isInvalid(context.representation, candidate.host));
  ...
  var PlayerMonitor = class {
    ...
    tick() {
      ...
      if (!disabled && watchdog === "recovering" && (this.#startupObservedProgress || !firstMediaAt) && now - this.#lastRecoveryAt >= 3e4) {
      ...
      if (!disabled && firstMediaAt && startupAge >= 15e3 && !this.#startupRescueAttempted && !this.#startupObservedProgress && video.available && !video.paused && !video.seeking && !video.ended && !video.mediaError && video.playableBufferSec < 1 && (video.readyState <= 1 || video.width === 0 && video.height === 0) && !this.recovery.isRecovering() && now >= this.#seekGraceUntil) {
