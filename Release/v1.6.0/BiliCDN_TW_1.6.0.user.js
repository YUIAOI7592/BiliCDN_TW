...
(() => {
  ...
  function createWorkerStats(deps) {
    ...
    const summarizeWorkerStats = /* @__PURE__ */ __name(() => {
      ...
      let verdict = deps.EnableWorkerIntercept ? "尚無資料（先播放影片數分鐘再查）" : "Worker 攔截目前停用；要量測需將 EnableWorkerIntercept 改為 true 後重整";
...
`, "moduleWorkerPatch");
        ...
        const handlePrivateWorkerReport = /* @__PURE__ */ __name((data) => {
          if (deps.disabled) return false;
        ...
        const registerWorker = /* @__PURE__ */ __name((worker, capability, blobUrl) => {
          ...
          try {
            ...
            channel.port1.onmessage = (event) => {
              handlePrivateWorkerReport(event && event.data);
            };
            try {
              channel.port1.start();
            } catch {
        ...
        unsafeWindow.Worker = (_a = class extends OriginalWorker {
  ...
  function createReport(deps) {
    ...
    const buildDiagReport = /* @__PURE__ */ __name(() => {
      ...
      const lines = [
        ...
        "Worker 量測：created=" + ws.created + " netCalls=" + ws.netCalls + " mediaSeen=" + ws.mediaSeen + " rewrites=" + ws.rewrites + " 觀察" + ws.observedDays + "天 判讀=" + ws.verdict
      ];
