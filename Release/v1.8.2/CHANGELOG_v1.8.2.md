...
- 媒體資格只接受已正規化且經原生 accessor 確認的 GET。非 GET Fetch 原樣轉送且不建立評級；XHR 使用不具原生 brand 的公開 facade，私有 backend 不暴露給頁面 prototype。原生呼叫固定使用捕捉的 `Reflect.apply`，改寫後的 Request 直接沿用已驗證 method，避免再次讀取頁面可變 prototype。
