...
(() => {
  ...
  var RouteCoordinator = class {
    ...
    apply(url, kindHint = null, demand) {
      ...
      if (!decision || decision.action === "block" || !this.#decisionAllowed(decision, context, kind)) decision = context ? this.#choose(context, playbackDemand, "request", null) : this.#catalogOnly(playbackDemand, parsed.host, kind);
    ...
    playerOutput(representation, original, decision, originals) {
      ...
      const backups = lockedOriginal ? [] : [.../* @__PURE__ */ new Set([...generated, ...originals.filter(allowed)])].filter((url) => url !== primary).slice(0, 5);
...
量測狀態：${this.deps.measurement.snapshot().reason}`;
    ...
    #readModel() {
      ...
      return Object.freeze({
        version: GM_info?.script?.version ?? "2.0.4",
