...
(() => {
  ...
  function createSettings(deps, input) {
    ...
    const VERSION = typeof GM_info !== "undefined" && GM_info.script && GM_info.script.version || "1.8.5";
  ...
  function createNativeRoutes(deps) {
    ...
    const routeEligible = /* @__PURE__ */ __name((group, route) => !!group && !group.ambiguous && !!route && (!deps.isHostAllowed || deps.isHostAllowed(route.host)) && (group.source !== "page-hint" || group.unlocked.has(route.url)), "routeEligible");
    const pageRepresentation = /* @__PURE__ */ __name((context) => {
      ...
      if (group.source !== "page-hint" || !group.unlocked.has(route.requestedUrl) && !catalogVerified) return null;
    ...
    const resolveRequestRouteUnchecked = /* @__PURE__ */ __name((url, context) => {
      ...
      if (exactSignedRoute && !plannedAffinity?.enforced && !avoidedHosts.has(exactSignedRoute.host) && plannedAffinity?.type !== "native-signed" && (!plannedAffinity || exactSignedRoute.host !== plannedAffinity.host)) {
        return {
    ...
    const chooseRecoveryRoute = /* @__PURE__ */ __name((group, failedHost) => {
      const fallback = [deps.resolvedCdn, deps.peekCurrentCdn(), group?.catalogFallback].find((host) => host && (!deps.isHostAllowed || deps.isHostAllowed(host))) || null;
    ...
    const beginRouteRecovery = /* @__PURE__ */ __name((reason, failedHost = null) => {
    ...
    const isRouteSampleAllowed = /* @__PURE__ */ __name((url, context = null) => {
      ...
      if (group) {
        ...
        return group.source === "page-hint" ? group.unlocked.has(parsed.url) && group.routes.some((route) => route.url === parsed.url) || group.verifiedCatalogUrls.has(parsed.url) && group.generatedUrls.has(parsed.url) : group.routes.some((route) => route.identity === parsed.identity);
      }
  ...
  function createPlayurl(deps) {
    const playInfoTransformer = /* @__PURE__ */ __name((playInfo, options = null) => {
      ...
      const transformList = /* @__PURE__ */ __name((list, isDash, kind = "muxed") => {
        ...
        list.forEach((item) => {
          if (nativeTransportSource && !isDash) deps.registerMediaRepresentation(
  ...
  function createApplication(deps) {
    ...
    const onSpaNavigate = /* @__PURE__ */ __name(() => {
      ...
      const pendingPagePlayInfo = assigned && assigned.serial > lastSpaBoundaryAssignmentSerial && assigned.value === currentValue && (assigned.key === key || currentValue !== currentPagePlayInfoValue) ? currentValue : null;
