...
(() => {
  ...
  function createSettings(deps, input) {
    ...
    const VERSION = typeof GM_info !== "undefined" && GM_info.script && GM_info.script.version || "1.9.2";
  ...
  function createNativeRoutes(deps) {
    ...
    let avoidedHosts = /* @__PURE__ */ new Map();
    const avoidHost = /* @__PURE__ */ __name((host, durationMs = Infinity) => {
      host = safeHost(host);
      if (!host) return false;
    ...
    const routeEligible = /* @__PURE__ */ __name((group, route) => !!group && !group.ambiguous && !!route && (!deps.isHostAllowed || deps.isHostAllowed(route.host)) && (!requiresTransportUnlock(group.source) || group.unlocked.has(route.url)), "routeEligible");
    ...
    const resolveRequestRouteUnchecked = /* @__PURE__ */ __name((url, context) => {
      ...
      if (exactSignedRoute && group.source !== "player-mpd" && !plannedAffinity?.enforced && !isHostAvoided(exactSignedRoute.host) && plannedAffinity?.type !== "native-signed" && (!plannedAffinity || exactSignedRoute.host !== plannedAffinity.host)) {
        return {
    ...
    const chooseRecoveryRoute = /* @__PURE__ */ __name((group, failedHost, options = null) => {
      ...
      const fallback = fallbackCandidates.find((host) => host && (fixed || !Number.isFinite(options?.temporaryMs) || host !== failedHost && !isHostAvoided(host)) && (!deps.isHostAllowed || deps.isHostAllowed(host))) || null;
  ...
  function createVideoCoreRecovery(deps) {
    ...
    const startReload = /* @__PURE__ */ __name((currentVideo, playback, liveness) => {
      const t = now();
      ...
      if (!deps.resolvedCdn) {
        try {
          failedHost = deps.getNativeRouteDiagnostics()?.currentHost || deps.getAttributedVideoHost?.() || null;
        } catch {
        }
        try {
          deps.beginRouteRecovery("video-init-dead", failedHost, { temporaryMs: 2 * 60 * 1e3 });
        } catch {
        }
      }
      record("reloading", {
    ...
    const beginResume = /* @__PURE__ */ __name((playback, snapshots) => {
      const t = now();
      resumeToken++;
      resume = {
        token: resumeToken,
    ...
    const tick = /* @__PURE__ */ __name((currentVideo, playback) => {
      ...
      const supportingEvidence = strong ? audioAdvanced || playback.bufferAheadSec > 0 || stagnant : audioAdvanced;
  ...
  function createSnapshot(deps) {
    ...
    const buildPublicDiagnosticSnapshot = /* @__PURE__ */ __name(() => {
      ...
      return deepFreezePublic({
        ...
        videoCore: {
          state: ["healthy", "waiting-metadata", "video-init-dead", "reloading", "recovered", "reload-failed", "breaker"].includes(videoCore.state) ? videoCore.state : "healthy",
