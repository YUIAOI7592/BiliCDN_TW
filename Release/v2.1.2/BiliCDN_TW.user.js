...
(() => {
  ...
  var RouteCoordinator = class {
    ...
    commitStartupChoice(url, selected, reason) {
      ...
      if (!context || context.generation !== selected.context.generation || context.epoch !== selected.context.epoch || context.representation !== selected.context.representation || this.#hardRestriction(selected.host, context.kind) || this.vault.isInvalid(context.representation, selected.host) || this.#startupIncompatible.get(mediaIdentity(selected.url) ?? "")?.has(selected.host)) return null;
      let decision;
  ...
  var urlOf = /* @__PURE__ */ __name((input) => typeof input === "string" ? input : "href" in input ? String(input.href) : input.url, "urlOf");
  ...
  var TransportAdapter = class {
    ...
    #installFetch() {
      ...
      const wrapped = /* @__PURE__ */ __name(async function(input, init) {
        ...
        const native = /* @__PURE__ */ __name(async (source) => {
          self.#stats.nativeCalled++;
          self.#noteNativeCall(activeRequest);
          const response2 = await Reflect.apply(original, this, [source, init]);
        ...
        if (isPlayurlApi(originalUrl)) {
          ...
          const response2 = await native(input);
          let text3;
          try {
        ...
        const sourceRequest = typeof input !== "string" && !("href" in input) ? input : null;
...
`;
  var ControlCenter = class {
    ...
    #renderOverview() {
      ...
      const mode = this.deps.routes.isOriginalComparison() ? "僅本分頁原始對照（只用合法原始 signed URL／不測速；禁止規則仍生效）" : settings.fixedHost ? `固定 ${settings.fixedHost}` : "自動";
...
量測狀態：${this.deps.measurement.snapshot().reason}`;
      ...
      for (const [kind, label] of [["video", "影片"], ["audio", "音訊"]]) {
        ...
        const stage = {
          planned: "僅計畫",
          "entered-hook": "已進入 hook（非網路確認）",
          "response-observed": "已觀察回應",
          "request-failed": "請求失敗"
        }[String(row?.stage)] ?? "未知";
        if (row) summary.textContent += `
${label}備援 ${row.actionId}：計畫 ${row.plannedHost}｜${stage}` + (row.responseHost ? `｜回應 ${row.responseHost} / ${row.status}` : "") + (row.outcome ? `｜${row.outcome}` : "");
      }
      body.append(summary);
    ...
    #renderSettings() {
      ...
      for (const host of TRUSTED_CATALOG) {
        ...
        const penalties = this.deps.restrictions.list().filter((row) => row.host === host).map((row) => `${row.type}（${row.kind === "all" ? "影片＋音訊" : row.kind === "video" ? "影片" : "音訊"}）`);
